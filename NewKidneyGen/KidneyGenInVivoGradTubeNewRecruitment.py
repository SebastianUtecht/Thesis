...
class Simulation:
    ...
    def chemo_atr_pot(self, x, attractors_mask, attractees_mask):
        ...
        with torch.no_grad():
            ...
            chemo_atr_idx   = chemo_atr_idx[:self.chemo_atr_N]
        ...
        chemo_dists     = chemo_dists[chemo_dists > 5.0]
    ...
    def time_step(self, x, p, q, p_mask, l, a, tstep):
        ...
        if (torch.sum(p_mask == 4) 
            + torch.sum(p_mask == 8)
            + torch.sum(p_mask == 3)
            + torch.sum(p_mask == 7))  < self.recruitment_stop and start_recruitment:
...
if __name__ == '__main__':
    ...
    sim_dict = {
        ...
        'UEC_l0'            : 0.4,
        'UEC_REC_str'       : 0.4,
        'primed_neigh_threshold': 4,
        'ABP_assim_threshold'   : 100,
        'l3iREC_MPC_min_atr' : True,
        'gamma'             : .01,
        'seethru'           : 0,
        ...
        'recruitment_stop'  : 30,
        'REC_IC_ratio'      : 1.0,
        'iREC_time_arr'     : [1000, 1000, 2000, 2640, 2000],
        'max_cells'         : 10_000,
