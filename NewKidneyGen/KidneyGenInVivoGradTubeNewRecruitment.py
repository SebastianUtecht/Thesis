...
class Simulation:
    def __init__(self, sim_dict):
        ...
        self.primed_neigh_threshold  = sim_dict['primed_neigh_threshold']
        self.ABP_assim_threshold     = sim_dict['ABP_assim_threshold']  
    ...
    def potential(self, x, p, q, p_mask, l, a, idx, d, tstep):
        ...
        #PROBLEM: THIS OVERWRITES P_MASK IN PLACE, THUS CHANGING THE ORIGINAL P_MASK
        #MAYBE FIXED?
        if self.assimilation:
            p_mask_true         = p_mask.clone() #used for implementing slightly different things for iREC after assimilation has begun   
        ...
        # Setting interaction between MPC and non-l3 activated iREC's such that the MPC's 'chase' after the iREC
        ...
        # DOESNT WORK AS IMPLEMENTED BUT WILL BE FIXED
    ...
    def time_step(self, x, p, q, p_mask, l, a, tstep):
        ...
        if len(self.iREC_timing) > 0:
            ...
            if torch.any(fin_REC_bool):
                ...
                p_mask[fin_REC_idx] = torch.where(p_mask[fin_REC_idx] == 3, 4, 8)
        ...
        #Check for new types
        if (torch.sum(p_mask == 4) + torch.sum(p_mask == 8))  < self.recruitment_stop:
            ...
            if self.assimilation:
                ...
                if torch.sum(p_mask == 3) > self.ABP_assim_threshold: #Before: torch.sum(l[p_mask == 3][:,1] > 0)
