...
class Simulation:
    ...
    def make_iREC_evolution(self, time_arr):
        ...
        alpha_ives      = torch.linspace(0, self.vesicle_alpha, i_ves_time, device=self.device)
    ...
    def potential(self, x, p, q, p_mask, l, a, idx, d):
        ...
        UEC_REC_mask = torch.logical_or( torch.sum(interaction_mask == torch.tensor([4,1], device=self.device), dim=2) == 2, MPC_iREC_mask)
        ...
        l_min[UEC_RECiREC_mask]  *= self.UEC_REC_str
        l_min[pUEC_RECiREC_mask] *= self.pUEC_REC_str
        ...
        pi_tilde[~aniso_mask] = pi[~aniso_mask] - a_min[~aniso_mask]*dx[~aniso_mask]
        pj_tilde[~aniso_mask] = pj[~aniso_mask] + a_min[~aniso_mask]*dx[~aniso_mask]
        ...
        pi_tilde[aniso_mask] = pi[aniso_mask] + a_min[aniso_mask]*angle_dx[aniso_mask]
        pj_tilde[aniso_mask] = pj[aniso_mask] - a_min[aniso_mask]*angle_dx[aniso_mask]
    ...
    def time_step(self, x, p, q, p_mask, l, a, tstep):
        ...
        if tstep < self.recruitment_stop:
            WNT_dists_squared = torch.sum(( self.activation_loc -  x)**2, dim=1)
            cells_within_act = WNT_dists_squared < self.activation_radius**2
            changing_cells   = torch.logical_and(cells_within_act, (p_mask == 0))
            changing_cells  *= (torch.exp(-WNT_dists_squared / (2*self.transform_sigma**2)) 
