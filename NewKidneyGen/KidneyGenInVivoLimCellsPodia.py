...
class Simulation:
    def __init__(self, sim_dict):
        ...
        self.MPC_seethru    = sim_dict['MPC_seethru']                                                           # Seethru for MPC cells
        ...
        self.l3iREC_l0iREC_atr_str  = sim_dict['l3iREC_l0iREC_atr_str']                                         # l0 value #COME BACK TO THIS             
        ...
        self.use_chemo_atr = False              # Bool for if the chemoattractant is used
        ...
        #Warnings
        if self.MPC_seethru and self.chemo_atr_str > 0:
            print("WARNING: MPC_seethru is active and chemoattractant is used. This might lead to unexpected behaviour")
        if self.MPC_seethru and not(self.l3iREC_l0iREC_atr_str):
            print("WARNING: MPC_seethru is active and l3iREC_l0iREC_atr_str is not. This doesn't make sense in the current model, as we want this interaction to be smaller than l0-l1 otherwise we have no proximal recruitment")
    ...
    # NOT WRITTEN SELF
    @staticmethod
    def find_potential_neighbours(x, k=100, distance_upper_bound=8, workers=-1):
        tree = cKDTree(x)
    ...
    # NOT WRITTEN SELF
    def find_true_neighbours(self, d, dx, idx=None, p_mask=None, l=None):
        with torch.no_grad():
            if self.MPC_seethru:
            ...
            while True:
                ...
                z_mask = torch.sum(n_dis < (d[i0:i1, :, None] ** 2 / 4), dim=2)
                if self.MPC_seethru:
                    #The used idx
                    int_mask_used   = interaction_mask[i0:i1]
                    li_used         = l_i[i0:i1]
                    lj_used         = l_j[i0:i1]

                    #Find early iREC interactions and primed cells
                    primed_mask = torch.any(int_mask_used == 6, dim=2)
                    li_non_polar_iREC_mask = torch.logical_and((~torch.any(li_used[:,:,1:], dim=2)) , torch.any(li_used[:,:,0]))
                    lj_non_polar_iREC_mask = torch.logical_and((~torch.any(lj_used[:,:,1:], dim=2)) , torch.any(lj_used[:,:,0]))
                    tot_non_polar_iREC_mask = torch.logical_or(li_non_polar_iREC_mask, lj_non_polar_iREC_mask)
                    tot_mask = torch.logical_or(primed_mask, tot_non_polar_iREC_mask)

                    seethru_tensor = torch.ones_like(z_mask, device=self.device, dtype=self.dtype) * self.seethru
                    seethru_tensor[tot_mask] = self.MPC_seethru

                    z_mask = z_mask <= seethru_tensor
                else:
                    z_mask = z_mask <= self.seethru
    ...
    def potential(self, x, p, q, p_mask, l, a, idx, d, tstep):
        ...
        z_mask = self.find_true_neighbours(d, dx, idx, p_mask)
        ...
        if torch.sum(allREC_mask) > 0:
            ...
            if self.l3_l1_adhesion_str != 1.0:
        ...
        if self.l3iREC_l0iREC_atr_str:
            ...
            mask1 = l_i[:,:,3] > 0.0
            mask2 = torch.logical_and((~torch.any(l_j[:,:,1:], dim=2)) , torch.any(l_j[:,:,0]))
            mask3 = torch.logical_and(mask1, mask2)

            mask4 = torch.logical_and((~torch.any(l_i[:,:,1:], dim=2)) , torch.any(l_i[:,:,0]))
            mask5 = l_j[:,:,3] > 0.0
            mask6 = torch.logical_and(mask4, mask5)

            tot_mask = torch.logical_or(mask3, mask6)

            primed_mask = torch.any(interaction_mask == 6, dim=2)

            tot_mask = torch.logical_or(tot_mask, primed_mask)
            ...
            l_min[tot_mask] = torch.tensor([self.l3iREC_l0iREC_atr_str, 0, 0, 0], device=self.device, dtype=self.dtype)
