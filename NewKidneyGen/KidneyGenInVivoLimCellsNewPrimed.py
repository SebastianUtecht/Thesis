...
class Simulation:
    def __init__(self, sim_dict):
        ...
        self.polar_time = torch.sum(self.iREC_times[:1]) + 1            # Time when the cells start to polarize
        self.i_ves_time_start = self.polar_time                         # Time when the vesicle formation starts
        self.i_ves_time_end   = torch.sum(self.iREC_times[:2]) + 1      # Time when the vesicle formation ends
        ...
        self.chemo_dist_deact  = sim_dict['chemo_dist_deact']       # Threshold for the number of only ABP cells a primed MPC cell can have as neighbours before it continues in its evolution
        self.chemo_atr_threshold     = sim_dict['chemo_atr_threshold']          # Number of iREC cells recruited before recruitment changes to facilitate new recruits attaching to the bottom of the forming nephron    
        self.use_assimilation        = sim_dict['use_assimilation']             # Boolean for if assimilation should be used
        ...
        self.using_chemo_atr = False              # Bool for if the chemoattractant is used
    ...
    def make_iREC_evolution(self, time_arr):
        ...
        pref_time       = int(time_arr[0] / self.dt)
        i_ves_time      = int(time_arr[1] / self.dt)
        ves_time        = int(time_arr[2] /self.dt)
        i_tube_time     = int(time_arr[3] /self.dt)
        ...
        non_polar_part  = torch.cat((non_polar_pref, non_polar_ives, non_polar_rest), axis=0)
        ...
        polar_ipref     = torch.zeros((pref_time,3), device=self.device)
        ...
        alpha_pref_ipref= torch.zeros(pref_time, device=self.device)
        ...
        return total, alpha_total, tot_time, torch.tensor([pref_time, i_ves_time, ves_time, i_tube_time], device=self.device, dtype=torch.int)
    ...
    def chemo_atr_pot(self, x, attractors_mask, attractees_mask):
        ...
        return torch.sum(chemo_pot), chemo_atr_pos
    ...
    def potential(self, x, p, q, p_mask, l, a, idx, d, tstep):
        ...
        if self.no_assim_WNT:
            tube_mask *= (~(p_mask_true ==  7)) * (~(p_mask_true ==  8))
        ...
        if self.using_chemo_atr and self.chemo_atr_str > 0.0:
            ...
            if torch.sum(attractees_mask)  > 0 and torch.sum(attractors_mask) > 0:
                chemo_pot, chemo_atr_dest = self.chemo_atr_pot(x=x, attractors_mask = attractors_mask, attractees_mask = attractees_mask)
                ...
                with torch.no_grad():
                    ...
                    _, chemo_atr_idx= torch.sort(WNT_x_dists, descending=True)
                    chemo_atr_idx   = chemo_atr_idx[:self.chemo_atr_N]                  #Maybe just use 5 attractors
                    chemo_atr_dest  = torch.mean(attractors_pos[chemo_atr_idx], dim=0)
        else:
            chemo_atr_dest = None #torch.tensor([0.0, 0.0, 0.0], device=self.device, dtype=self.dtype)
        ...
        return Vij_sum, int(m), idx, chemo_atr_dest
    ...
    def time_step(self, x, p, q, p_mask, l, a, tstep):
        ...
        # Calculate potential
        V, self.true_neighbour_max, idx, chemo_atr_dest = self.potential(x, p, q, p_mask, l, a, idx, d, tstep=tstep)
        ...
        end_recruitment = torch.sum(p_mask == 4) + torch.sum(p_mask == 3)
        if self.use_assimilation:
            end_recruitment += torch.sum(p_mask == 7) + torch.sum(p_mask == 8)   
            
        end_recruitment = end_recruitment >= self.recruitment_stop
        
        if (end_recruitment and not self.recruitment_stopped) and self.use_assimilation:
            p_mask[p_mask == 6] = 0
        ...
        if start_recruitment and not end_recruitment:
            ...
            if self.assimilation:
                new_primed_cells_idx = torch.argwhere(changing_cells)[:,0]
                p_mask[new_primed_cells_idx] = 6
                
                primed_cells_idx = torch.argwhere(p_mask == 6)[:,0]
                primed_cells_pos = x[primed_cells_idx]
                primed_cell_close_to_dest = torch.sum((primed_cells_pos - chemo_atr_dest)**2, dim=1) < self.chemo_dist_deact**2
                changing_cells_idx = torch.argwhere(p_mask == 6)[:,0][primed_cell_close_to_dest]

            else: 
                changing_cells_idx = torch.argwhere(changing_cells)[:,0]
                if not self.using_chemo_atr:
                    num_l1_cells = torch.sum(torch.any(l[p_mask == 3][:,1:], dim=1))
                    num_l1_cells += torch.sum(p_mask == 4)
                    if num_l1_cells > self.chemo_atr_threshold: #(torch.sum(p_mask == 3) + torch.sum(p_mask == 4) ) > self.chemo_atr_threshold: #Before: torch.sum(l[p_mask == 3][:,1] > 0)
                        self.using_chemo_atr = True
                        if self.use_assimilation:
...
def run_simulation(sim_dict):
    ...
    for xx, pp, qq, pp_mask, ll, aa in itertools.islice(runner, yield_steps):
        i += 1
        if verbose:
            print(f'Running {i} of {yield_steps}   ({yield_every * i} of {yield_every * yield_steps})   ({len(xx)} cells)', end='\n')
