...
class Simulation:
    ...
    def get_prolif_positions(self, p, q, p_mask):
        ...
        with torch.no_grad():
            ...
            if torch.numel(p_polar) == 3:
                ...
                new_pos[0] = 3.0 #torch.normal(mean = 0.0, std = 1.0, size=(1,1)).squeeze()
                ...
                new_pos[:,0] = 3.0 #torch.normal(mean = 0.0, std = 1.0, size=(p_polar.shape[0],1)).squeeze()
    ...
    def cell_division(self, x, p, q, p_mask, beta_decay = 1.0):
        ...
        within_sphere = torch.sum((torch.tensor([0.0,-15.0,25.0], device=self.device) -  x)**2, dim=1) < 20.0**2
        ...
        if torch.sum(events) > 0:
            with torch.no_grad():
                ...
                p = torch.cat((p, p0))
                q = torch.cat((q, q0))
                p_mask = torch.cat((p_mask, torch.zeros(len(x0), device=self.device)))
                beta = torch.cat((beta, torch.zeros(len(x0), device=self.device)))
